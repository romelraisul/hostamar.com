...
export async function GET(req: NextRequest) {
  ...
  try {
    // Ensure tables exist (no prisma db push needed — runtime self-heal)
    try {
      await (prisma as any).$executeRawUnsafe(`
        CREATE TABLE IF NOT EXISTS "TvChannelStability" (
          "id" TEXT PRIMARY KEY,
          ...
          "avgLoadTimeMs" INTEGER NOT NULL DEFAULT 9999,
          "lastSuccessAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
          "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP
        );
        CREATE INDEX IF NOT EXISTS "TvChannelStability_stabilityScore_popularityScore_idx" ON "TvChannelStability"("stabilityScore" DESC, "popularityScore" DESC);
