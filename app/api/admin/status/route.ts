...
export async function GET() {
  const [tv, login, db] = await Promise.all([
    checkHttp('https://tv.hostamar.com/master.m3u8'),
    checkHttp('https://hostamar.com/login'),
  ...
  const components = [
    ...
    { status: login === 200 ? '✅' : '❌', component: 'Login Page', value: login === 200 ? '200 OK' : `${login} DOWN`, detail: 'hostamar.com/login (sso_callback_failed fixed)', verified: `HTTP ${login}` },
