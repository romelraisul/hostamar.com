 ...
 * Admin approves a PENDING TrxID submission → VERIFIED + credits + subscription.
...
export async function POST(
  ...
  try {
    ...
    if (v.status === 'REJECTED') {
      return NextResponse.json({ error: 'ALREADY_REJECTED', message: 'Cannot approve a rejected verification.' }, { status: 409 })
    }

    const body = await req.json().catch(() => ({}))
    const note = body.note ? String(body.note).slice(0, 500) : null

    // Reset to PENDING so grantPaymentBenefits runs (it skips non-PENDING-safe states)
    await prisma.paymentVerification.update({
      where: { id: v.id },
      data: { status: 'PENDING', reviewedBy: admin.email, reviewNote: note },
    })
    ...
    return NextResponse.json({
      ...
      message: 'Payment approved. Credits granted and subscription activated.',
    })
  } catch (err: any) {
