...
import { validatePromo, discountPrice } from '@/lib/promo';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { plan, method, phone, walletAddress, promoCode } = body as {
      plan: string;
      method: string;
      phone?: string;
      walletAddress?: string;
    ...
    // Apply promo discount (if any). Validation mirrors the UI to prevent
    // prices going negative or arbitrary user input controlling amount.
    const promo = validatePromo(promoCode);
    const finalAmount = discountPrice(planInfo.amount, promo);
    ...
    if (m === 'bkash') {
      paymentResult = await createBkashPayment(finalAmount, trxId, phone!);
    } else if (m === 'nagad') {
      paymentResult = await createNagadPayment(finalAmount, trxId, phone!);
    } else if (m === 'rocket') {
      paymentResult = await createRocketPayment(finalAmount, trxId, phone!);
    } else {
      paymentResult = await createUSDTPayment(finalAmount, trxId, walletAddress!);
    }
    ...
    const instructions = generateInstructions(m, { ...planInfo, amount: finalAmount }, trxId, phone, walletAddress);
