...
/**
 * /docs — Hostamar customer documentation (V15): 70k-word foundation,
 * structure for 1M (106 products expandable ×9k words each).
 * Sticky left sidebar (106-product sections), right TOC, search,
 * 1cr=1TK=1COIN banner, bKash 01822417463. Public — no auth.
 * Bangla + English content per i18n (t() where keys exist).
 */
...
export default function DocsPage() {
  const sections = (content as any).sections as Section[]
    ...
    <div className="min-h-screen bg-white text-zinc-900">
        ...
        <div className="mx-auto flex max-w-[1500px] items-center gap-3 px-4 py-3">
          ...
          <div className="relative ml-auto hidden w-72 sm:block">
            ...
            <input
              ...
              placeholder="ডকস সার্চ করুন… (search docs)"
      ...
      <div className="mx-auto grid max-w-[1500px] gap-8 px-4 py-8 lg:grid-cols-[270px_minmax(0,1fr)_220px]">
        ...
        <aside className={`z-30 lg:sticky lg:top-[64px] lg:block lg:h-[calc(100vh-90px)] lg:overflow-y-auto lg:pr-3 ${sidebarOpen ? 'fixed inset-x-0 top-[58px] bottom-0 overflow-y-auto bg-white p-4' : 'hidden'}`}>
          <p className="mb-2 text-xs font-bold text-zinc-400">সেকশনস / SECTIONS</p>
        ...
        <main className="min-w-0">
          <div className="mb-6">
            <h1 className="text-2xl font-bold">Hostamar Customer Manual — সম্পূর্ণ গাইড</h1>
          ...
          {q && (
            <p className="mb-4 rounded-xl bg-[#ECFDF5] px-3 py-2 text-xs" style={{ color: GREEN }}>
              সার্চ: “{q}” — {filtered.length} সেকশন পাওয়া গেছে
            </p>
          )}
          {filtered.map((s) => (
          ...
          {filtered.length === 0 && <p className="py-10 text-center text-sm text-zinc-400">কিছু পাওয়া যায়নি — অন্য শব্দে চেষ্টা করুন</p>}
        </main>

        {/* ── RIGHT: TOC (active section) ── */}
        <aside className="sticky top-[64px] hidden h-[calc(100vh-90px)] overflow-y-auto xl:block">
          <p className="mb-2 text-xs font-bold text-zinc-400">এই পেজে / ON THIS PAGE</p>
