...
export default function ModelsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <p className="text-sm text-[#57534E]">source:{source||'kv'} • {models.length} models • free {freeCount} • Tier S (Reasoning) kimi-k3 1M, Tier A Long 1M longcat 1,048,756 + gemini 1M, Tier B Balanced</p>
      ...
      {selModel && (
        <div className="mt-4 rounded-xl border bg-[#FFFDF6] p-4">
          <p className="font-semibold">{selModel.displayName} <PongBadge id={selModel.id} /> <span className="text-xs text-[#57534E]">[{selModel.context}] ctx {selModel.context_length?.toLocaleString()} • {selModel.provider} • {selModel.free?'FREE':'PAID 402'}</span></p>
          <p className="text-xs text-[#57534E]">Tier {tierOf(selModel)} • upstream {selModel.provider} • strip [ctx]+opencode/ → {selected.replace(/\s*\[[^\]]*\]\s*$/, '').replace(/^opencode\//,'')}</p>
      ...
      <div className="mt-6 grid gap-2 md:grid-cols-3">
        <div className="rounded-xl border bg-[#FFFDF6] p-3"><h3 className="font-semibold text-sm">Tier S Reasoning</h3><p className="text-xs text-[#57534E]">kimi-k3 1M • deepseek-v4 1.3M • Thinking</p></div>
        <div className="rounded-xl border bg-[#FFFDF6] p-3"><h3 className="font-semibold text-sm">Tier A Long Context</h3><p className="text-xs text-[#57534E]">longcat 1,048,756 • x-preview 1,048,576 • gemini 1M • ox-alpha PONG</p></div>
        <div className="rounded-xl border bg-[#FFFDF6] p-3"><h3 className="font-semibold text-sm">Tier B Balanced</h3><p className="text-xs text-[#57534E]">512K-256K • fast & cheap • 0.03-0.09 Taka/msg</p></div>
      </div>
    </div>
  )
}