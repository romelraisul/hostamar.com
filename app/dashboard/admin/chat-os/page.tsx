...
export default function ChatOsPage() {
  ...
  return (
    <div className="flex h-screen flex-col bg-[#FBF4E4] text-[#1C1917]" style={{ fontSize: 13 }}>
      {/* ── TOP BAR ── */}
      <header className="flex items-center gap-3 border-b border-[#D8CDB4] px-3 py-2">
        <span className="font-bold">Hostamar Chat OS</span>
        <span className="rounded-full bg-[#FFFDF6] px-2 py-0.5 text-[10px] text-[#78716C]">Orca IDE</span>
        <div className="flex items-center gap-1 text-[11px] text-[#78716C]">
          <select className="rounded border border-[#D8CDB4] bg-[#FDF8EC] px-1.5 py-0.5 text-[11px]">
        ...
        <GitBranch className="h-3.5 w-3.5 text-[#57534E]" /><span className="text-[11px] text-[#78716C]">{branch}</span>
        <div className="ml-auto flex items-center gap-3">
          <span className="text-[10px] text-[#57534E]">bKash 01822417463</span>
          <span className="flex items-center gap-1 rounded-full bg-[#FFFDF6] px-2 py-0.5 text-[11px]">
            <Coins className="h-3 w-3" style={{ color: GREEN }} />
            {credits != null ? credits.toLocaleString('bn-BD') : '…'} cr
            <span className="h-1 w-8 overflow-hidden rounded-full bg-[#FDF8EC]"><span className="block h-full" style={{ width: `${pct}%`, background: GREEN }} /></span>
          </span>
          <select value={model} onChange={e => setModel(e.target.value)} className="max-w-40 rounded border border-[#D8CDB4] bg-[#FDF8EC] px-1.5 py-0.5 text-[11px]">
            {models.map(m => <option key={m.id}>{m.id}{m.free ? ' ✓free' : ''}</option>)}
          </select>
          <span className="rounded-full bg-[#FFFDF6] px-2 py-0.5 text-[10px] text-[#78716C]">MCP {mcpTools.length}</span>
          <span className="rounded-full bg-[#FFFDF6] px-2 py-0.5 text-[10px] text-[#78716C]">WebMCP ✓</span>
        </div>
      </header>
      ...
      <div className="flex min-h-0 flex-1">
        {/* LEFT: File Explorer */}
        <aside className="flex w-56 flex-col border-r border-[#D8CDB4]" style={{ minWidth: 180 }}>
          <div className="flex items-center justify-between px-2 py-1.5 text-[11px] font-semibold text-[#78716C]">
            <span className="flex items-center gap-1"><Files className="h-3.5 w-3.5" /> FILES</span>
            <button onClick={newFile} title="New file (1cr on save)"><Plus className="h-3.5 w-3.5 hover:text-[#1C1917]" /></button>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-1">
            {files.length === 0 && <p className="px-2 py-1 text-[10px] text-[#57534E]">চ্যাটে ফাইল বানাতে বলুন, বা + চাপুন</p>}
            {files.map(f => (
              <div key={f.name} className="group flex items-center">
                <button onClick={() => openF(f.name)} className={`flex-1 truncate rounded px-2 py-1 text-left text-[11px] hover:bg-[#FFFDF6] ${openFile === f.name ? 'bg-[#FFFDF6] text-[#1C1917]' : 'text-[#57534E]'}`}>
                  📄 {f.name}
                </button>
                <button onClick={() => delF(f.name)} className="hidden px-1 group-hover:block"><Trash2 className="h-3 w-3 text-[#57534E] hover:text-red-400" /></button>
              </div>
            ))}
          </div>
          {openFile && (
            <div className="border-t border-[#D8CDB4] p-1">
              <button onClick={saveF} disabled={!dirty} className="flex w-full items-center justify-center gap-1.5 rounded-lg py-1.5 text-[11px] font-semibold disabled:bg-[#FFFDF6] disabled:text-[#57534E]" style={{ background: dirty ? GREEN : undefined }}>
        ...
        <main className="flex min-w-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
            {msgs.map((m, i) => (
              <div key={i} className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-3 py-2 ${m.role === 'user' ? 'ml-auto bg-[#0E7C3A] text-white' : m.role === 'system' ? 'mx-auto bg-[#FDF8EC] text-[#78716C]' : 'bg-[#FFFDF6] text-[#1C1917]'}`}>
                {m.content}
                {m.meta && <p className="mt-1 text-[9px] text-[#57534E]">{m.meta}</p>}
              </div>
            ))}
            {busy && <div className="flex items-center gap-2 text-[11px] text-[#57534E]"><Loader2 className="h-3.5 w-3.5 animate-spin" /> AI thinking…</div>}
            <div ref={endRef} />
          </div>
          <div className="border-t border-[#D8CDB4] p-2">
            <div className="flex items-end gap-2">
              <textarea
                ...
                className="flex-1 resize-none rounded-xl border border-[#D8CDB4] bg-[#FDF8EC] px-3 py-2 text-[12px] focus:border-[#0E7C3A] focus:outline-none"
              />
              <button onClick={send} disabled={busy || !input.trim()} className="rounded-xl p-2.5 disabled:bg-[#FFFDF6]" style={{ background: GREEN }}>
                <Send className="h-4 w-4 text-[#1C1917]" />
              </button>
            </div>
          </div>
        </main>
        ...
        <aside className="hidden w-72 flex-col border-l border-[#D8CDB4] lg:flex" style={{ minWidth: 260 }}>
          <div className="flex items-center justify-between px-2 py-1.5 text-[11px] font-semibold text-[#78716C]">
            <span className="flex items-center gap-1"><MonitorPlay className="h-3.5 w-3.5" /> PREVIEW</span>
            <div className="flex items-center gap-1">
              <button onClick={startPreview} title="Start session (ফ্রি)" className="flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] hover:bg-[#FFFDF6]"><Play className="h-3 w-3" /> ফ্রি</button>
              <button onClick={() => setDesignMode(v => !v)} title="Design Mode: click any element → drops into chat (1cr per click)"
                className={`flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] ${designMode ? 'font-bold text-[#1C1917]' : 'hover:bg-[#FFFDF6]'}`}
          ...
          <div className="border-t border-[#D8CDB4] p-1.5">
            <textarea
              value={previewHtml}
              onChange={e => setPreviewHtml(e.target.value)}
              rows={4}
              spellCheck={false}
              className="w-full resize-none rounded-lg border border-[#D8CDB4] bg-[#FDF8EC] p-1.5 font-mono text-[9px] text-[#57534E] focus:outline-none"
            />
            <p className="mt-1 text-[9px] text-[#57534E]">{designMode ? 'Design Mode: preview-এ যেকোনো element ক্লিক করুন → চ্যাটে যাবে (1cr)' : 'Design Mode off'}</p>
          </div>
        </aside>
      </div>

      {/* ── BOTTOM: Terminal / Git / MCP / Plugins+Tasks ── */}
      <section className="h-64 shrink-0 border-t border-[#D8CDB4]" style={{ minHeight: 160 }}>
        <div className="flex items-center gap-1 border-b border-[#D8CDB4] px-2">
          {([['terminal', TerminalIcon, 'TERMINAL'], ['git', GitBranch, 'SOURCE CONTROL'], ['mcp', Boxes, 'MCP SERVERS'], ['plugins', Plug, 'PLUGINS + TASKMASTER']] as const).map(([id, Icon, label]) => (
            <button key={id} onClick={() => setTab(id as Tab)}
              className={`flex items-center gap-1 border-b-2 px-2.5 py-1.5 text-[10px] font-semibold ${tab === id ? 'border-[#0E7C3A] text-[#1C1917]' : 'border-transparent text-[#57534E] hover:text-[#57534E]'}`}>
        ...
        {tab === 'terminal' && (
          <div className="flex h-[calc(100%-29px)] flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto bg-[#FDF8EC] p-2 font-mono text-[11px] leading-relaxed text-lime-300">
              {termLines.map((l, i) => <div key={i} className="whitespace-pre-wrap">{l}</div>)}
            </div>
            <div className="flex items-center gap-1 border-t border-[#D8CDB4] px-2 py-1.5">
              ...
              <input value={termCmd} onChange={e => setTermCmd(e.target.value)}
                ...
                className="flex-1 bg-transparent font-mono text-[11px] text-[#1C1917] focus:outline-none" />
            </div>
          </div>
        )}
        ...
        {tab === 'git' && (
          <div className="flex h-[calc(100%-29px)] flex-col p-2">
            <div className="flex items-center gap-2">
              <button onClick={() => git('status')} className="flex items-center gap-1 rounded-lg border border-[#D8CDB4] px-2 py-1 text-[10px] hover:bg-[#FFFDF6]"><RefreshCw className="h-3 w-3" /> Status</button>
              <button onClick={() => git('diff')} className="flex items-center gap-1 rounded-lg border border-[#D8CDB4] px-2 py-1 text-[10px] hover:bg-[#FFFDF6]">Diff</button>
              <input value={commitMsg} onChange={e => setCommitMsg(e.target.value)} placeholder="commit message"
                className="w-52 rounded-lg border border-[#D8CDB4] bg-[#FDF8EC] px-2 py-1 text-[10px] focus:outline-none" />
              <button onClick={() => git('commit')} className="flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-semibold text-[#1C1917]" style={{ background: GREEN }}>✓ Commit (ফ্রি)</button>
            </div>
            <pre className="mt-2 min-h-0 flex-1 overflow-auto rounded-lg bg-[#FDF8EC] p-2 font-mono text-[10px] text-[#57534E]">{gitOut || 'git status / diff দেখুন — commit করুন Chat OS থেকেই'}</pre>
          </div>
        )}
        ...
        {tab === 'mcp' && (
          <div className="h-[calc(100%-29px)] overflow-y-auto p-2">
            <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-4">
              {mcpTools.map(t => (
                <button key={t.name} onClick={() => callTool(t)} className="rounded-xl border border-[#D8CDB4] p-2 text-left hover:border-[#0E7C3A]">
                  <p className="truncate text-[10px] font-bold text-[#1C1917]">{t.name}</p>
                  <p className="truncate text-[9px] text-[#57534E]">{t.server}</p>
        ...
        {tab === 'plugins' && (
          <div className="grid h-[calc(100%-29px)] grid-cols-2 gap-2 p-2">
            <div className="min-h-0 overflow-y-auto">
              <p className="mb-1 text-[10px] font-bold text-[#78716C]">PLUGINS</p>
              {plugins.map(p => (
                <div key={p.id} className="flex items-center justify-between rounded-lg border border-[#D8CDB4] px-2 py-1.5">
                  <div><p className="text-[10px] font-semibold text-[#1C1917]">{p.name}</p><p className="text-[9px] text-[#57534E]">{p.desc}</p></div>
                  {p.installed ? <span className="text-[9px] text-emerald-400">✓ active</span>
                    : <button onClick={() => installPlugin(p.id)} className="rounded px-1.5 py-0.5 text-[9px] font-semibold text-[#1C1917]" style={{ background: GREEN }}>Install (ফ্রি)</button>}
                </div>
              ))}
            </div>
            <div className="flex min-h-0 flex-col">
              <p className="mb-1 text-[10px] font-bold text-[#78716C]">TASKMASTER</p>
              <div className="mb-1 flex gap-1">
                <input value={taskTitle} onChange={e => setTaskTitle(e.target.value)} placeholder="নতুন টাস্ক (ফ্রি)"
                  className="flex-1 rounded-lg border border-[#D8CDB4] bg-[#FDF8EC] px-2 py-1 text-[10px] focus:outline-none" />
                <button onClick={addTask} className="rounded-lg px-2 text-[10px] font-semibold text-[#1C1917]" style={{ background: GREEN }}>+</button>
              </div>
              <div className="min-h-0 flex-1 overflow-y-auto">
                {tasks.map(t => (
                  <div key={t.id} className="flex items-center gap-1.5 rounded-lg border border-[#D8CDB4] px-2 py-1.5 text-[10px]">
                    <ListTodo className="h-3 w-3 text-[#57534E]" /> <span className="truncate text-[#57534E]">{t.title}</span>
                    <span className="ml-auto rounded-full bg-[#FFFDF6] px-1.5 text-[9px] text-[#78716C]">{t.status}</span>
                  </div>
                ))}
                {tasks.length === 0 && <p className="px-1 text-[9px] text-[#57534E]">কোনো টাস্ক নেই</p>}
              </div>
            </div>
          </div>
        )}
