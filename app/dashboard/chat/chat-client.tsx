      ...
      <aside className={`${modelsOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-20 w-72 bg-[#FFFDF6] border-r flex flex-col transition-transform`}>
        <div className="px-3 py-3 border-b space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold tracking-wide text-[#1C1917] flex items-center gap-1.5">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-3">
          {!models.length && <p className="px-2 py-6 text-center text-xs text-[#78716C]">মডেল লোড হচ্ছে...</p>}
          {grouped.map(([prov, list]) => (
            <div key={prov}>
              <p className="px-2 pb-1 text-[10px] font-bold tracking-wider text-[#78716C] uppercase">{prov} ({list.length})</p>
              <div className="space-y-0.5">
                {list.map(m => (
                  <button
                    key={m.id}
                    onClick={() => { setModelId(m.id); setModelsOpen(false) }}
                    className={`w-full text-left rounded-lg px-2.5 py-1.5 text-xs transition flex items-center justify-between gap-2 ${modelId === m.id ? 'bg-[#0E7C3A] text-white' : 'hover:bg-[#F1F5F9] text-[#1C1917]'}`}
                  >
                    <span className="truncate flex-1">{m.displayName}<PongBadge id={m.id} /></span>
                    <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold bg-amber-100 text-amber-700 ${modelId === m.id ? '!bg-white/20 !text-[#1C1917]' : ''}`}>PAID</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="border-t px-3 py-2.5 text-[10px] text-[#78716C] leading-relaxed">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between gap-2 border-b bg-[#FFFDF6] px-4 py-2.5 lg:px-6">
          <div className="flex items-center gap-2 min-w-0 pl-8 lg:pl-0">
            ...
            <span className="text-sm font-bold text-[#1C1917] truncate">{sel?.displayName || modelId}</span>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5 lg:px-8 space-y-4">
          {msgs.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center gap-2 text-[#57534E] px-6">
              <div className="w-12 h-12 rounded-2xl bg-[#ECFDF5] grid place-items-center"><Sparkles className="w-6 h-6 text-[#0E7C3A]" /></div>
              <p className="text-sm font-semibold text-[#1C1917]" style={{ fontFamily: "'Hind Siliguri', 'Noto Sans Bengali', sans-serif" }}>চ্যাট শুরু করুন</p>
          ...
          {busy && (
            <div className="flex gap-2.5">
              ...
              <div className="rounded-2xl bg-[#FFFDF6] border px-4 py-3 text-xs text-[#57534E] flex items-center gap-1 shadow-sm">
        ...
        <aside className="hidden xl:flex w-80 bg-[#FFFDF6] border-l flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-5 text-sm">
            ...
            <div className="rounded-xl border bg-[#F8FAFC] p-3">
              ...
              <p className="mt-1.5 text-lg font-black text-[#0F172A]">{bal ? bal.credits.toLocaleString() : '—'} <span className="text-xs font-normal text-[#57534E]">Taka</span></p>
              <p className="text-[11px] text-[#57534E]">{bal ? `≈ $${bal.usd.toFixed(2)}` : 'loading'} • Video 100 • Chat 1 • IDE 10</p>
            </div>
            <p className="text-[10px] text-[#78716C] leading-relaxed">Settings apply to next message. Free models only — paid 402 blocked at gateway (no card needed).</p>
          </div>
        </aside>
      )}
    </div>
