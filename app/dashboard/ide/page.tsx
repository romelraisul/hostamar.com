  ...
  return (
    <div className="flex h-screen flex-col bg-[#FBF4E4] text-[#1C1917]" style={{ fontSize: 13 }}>
      ...
      <header className="flex items-center gap-3 border-b border-[#D8CDB4] px-3 py-2">
        ...
        <span className="rounded-full bg-[#FFFDF6] px-2 py-0.5 text-[10px] text-[#78716C]">Orca ADE</span>
        <span className="text-[11px] text-[#78716C]">Worktrees: {worktrees.length}</span>
        <GitBranch className="h-3.5 w-3.5 text-[#57534E]" /><span className="text-[11px] text-[#78716C]">main</span>
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <select value={model} onChange={e => { setModel(e.target.value); fetch(`/api/orca/price?model=${encodeURIComponent(e.target.value)}`).then(r => r.json()).then(d => setPriceLbl(d.label || '')) }}
            className="max-w-44 rounded border border-[#D8CDB4] bg-[#FFFDF6] px-1.5 py-0.5 text-[11px]">
            {models.map(m => <option key={m.id}>{m.id} • PAID</option>)}
          </select>
          {priceLbl && <span className="rounded-full bg-[#FFFDF6] px-2 py-0.5 text-[9px] text-[#78716C]">{priceLbl}</span>}
          ...
          <User className="h-4 w-4 text-[#57534E]" />
        </div>
      </header>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside className="flex w-52 flex-col border-r border-[#D8CDB4]" style={{ minWidth: 190 }}>
          <div className="flex items-center justify-between px-2 py-1.5 text-[11px] font-semibold text-[#78716C]">
            <span>WORKSPACES</span>
            <button onClick={createWt} title="New worktree (5cr)"><Plus className="h-3.5 w-3.5 hover:text-[#1C1917]" /></button>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-1">
            {worktrees.length === 0 && <p className="px-2 py-1 text-[10px] text-[#57534E]">+ চেপে নতুন isolated worktree খুলুন (5cr) — প্রতি এজেন্ট আলাদা worktree-তে প্যারালাল কাজ করবে</p>}
            {worktrees.map(w => (
              <button key={w.id} onClick={() => setActiveWt(w)}
                className={`block w-full rounded px-2 py-1 text-left text-[10px] hover:bg-[#FFFDF6] ${activeWt?.id === w.id ? 'bg-[#FFFDF6] text-[#1C1917]' : 'text-[#57534E]'}`}>
                <GitMerge className="mr-1 inline h-3 w-3" />
                {w.name}
                <span className="block text-[9px] text-[#57534E]">{w.agent} • {w.status}</span>
              </button>
            ))}
          </div>
          <div className="border-t border-[#D8CDB4] p-1.5">
            <div className="flex items-center gap-1">
              <input type="number" min={1} max={5} value={fanCount} onChange={e => setFanCount(Math.max(1, Math.min(5, +e.target.value)))}
                className="w-12 rounded border border-[#D8CDB4] bg-[#FFFDF6] px-1 py-0.5 text-[10px]" />
              <button onClick={fan} disabled={busy} className="flex-1 rounded-lg py-1 text-[10px] font-semibold disabled:bg-[#FFFDF6]"
                style={{ background: busy ? undefined : GREEN, color: busy ? '#666' : '#fff' }}>
                ⚡ Fan ×{fanCount}
              </button>
            </div>
            <p className="mt-1 text-[9px] text-[#57534E]">1 prompt → {fanCount} agents → merge winner (5cr/worktree)</p>
          </div>
        </aside>

        {/* FILE EXPLORER */}
        <aside className="flex w-48 flex-col border-r border-[#D8CDB4]" style={{ minWidth: 170 }}>
          <div className="flex items-center justify-between px-2 py-1.5 text-[11px] font-semibold text-[#78716C]">
            ...
            <button onClick={async () => { const n = prompt('নতুন ফাইল?'); if (n) { const d = await osApi('file_save', { path: n, content: '' }); afterAction(d); loadFiles() } }}>
              <Plus className="h-3.5 w-3.5 hover:text-[#1C1917]" />
            </button>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-1">
            {files.map(f => (
              <div key={f.name} className="group flex items-center">
                ...
                <button onClick={async () => { if (confirm(`${f.name} ডিলিট?`)) { await osApi('terminal', { command: `rm ${f.name}` }); loadFiles() } }} className="hidden px-1 group-hover:block">
                  <Trash2 className="h-3 w-3 text-[#57534E] hover:text-red-400" />
                </button>
              </div>
            ))}
          </div>
          {openFile && (
            <div className="border-t border-[#D8CDB4] p-1">
              <button onClick={saveF} disabled={!dirty} className="flex w-full items-center justify-center gap-1.5 rounded-lg py-1.5 text-[11px] font-semibold disabled:bg-[#FFFDF6] disabled:text-[#57534E]"
        ...
        <main className="flex min-w-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
            {msgs.map((m, i) => (
              <div key={i} className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-3 py-2 ${m.role === 'user' ? 'ml-auto bg-[#0E7C3A] text-white' : m.role === 'system' ? 'mx-auto bg-[#FFFDF6] text-[#78716C]' : 'bg-[#FFFDF6] text-[#1C1917]'}`}>
                {m.content}
                {m.meta && <p className="mt-1 text-[9px] text-[#57534E]">{m.meta}</p>}
              </div>
            ))}
            {busy && <div className="flex items-center gap-2 text-[11px] text-[#57534E]"><Loader2 className="h-3.5 w-3.5 animate-spin" /> agent thinking…</div>}
            <div ref={endRef} />
          </div>
          <div className="border-t border-[#D8CDB4] p-2">
            <div className="flex items-end gap-2">
              <textarea value={input} onChange={e => setInput(e.target.value)}
                ...
                className="flex-1 resize-none rounded-xl border border-[#D8CDB4] bg-[#FFFDF6] px-3 py-2 text-[12px] focus:border-[#0E7C3A] focus:outline-none" />
        ...
        <aside className="hidden w-72 flex-col border-l border-[#D8CDB4] lg:flex" style={{ minWidth: 250 }}>
          <div className="flex items-center justify-between px-2 py-1.5 text-[11px] font-semibold text-[#78716C]">
          ...
          <div className="border-t border-[#D8CDB4] p-1.5">
            <textarea value={previewHtml} onChange={e => setPreviewHtml(e.target.value)} rows={4} spellCheck={false}
              className="w-full resize-none rounded-lg border border-[#D8CDB4] bg-[#FFFDF6] p-1.5 font-mono text-[9px] text-[#57534E] focus:outline-none" />
            <p className="mt-1 text-[9px] text-[#57534E]">{designMode ? 'Design Mode: element ক্লিক → chat (1cr)' : 'Design Mode off'}</p>
          </div>
        </aside>
      </div>

      {/* BOTTOM */}
      <section className="h-60 shrink-0 border-t border-[#D8CDB4]" style={{ minHeight: 140 }}>
        ...
        {tab === 'terminal' && (
          <div className="flex h-[calc(100%-29px)] flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto bg-[#FFFDF6] p-2 font-mono text-[11px] leading-relaxed text-lime-300">
        ...
        {tab === 'git' && (
          <div className="flex h-[calc(100%-29px)] flex-col p-2">
            <div className="flex items-center gap-2">
              <button onClick={() => git('status')} className="rounded-lg border border-[#D8CDB4] px-2 py-1 text-[10px] hover:bg-[#FFFDF6]">Status</button>
              <button onClick={() => git('diff')} className="rounded-lg border border-[#D8CDB4] px-2 py-1 text-[10px] hover:bg-[#FFFDF6]">Diff</button>
              <input value={commitMsg} onChange={e => setCommitMsg(e.target.value)} placeholder="commit message"
                className="w-52 rounded-lg border border-[#D8CDB4] bg-[#FFFDF6] px-2 py-1 text-[10px] focus:outline-none" />
              <button onClick={() => git('commit')} className="rounded-lg px-2 py-1 text-[10px] font-semibold text-[#1C1917]" style={{ background: GREEN }}>✓ Commit (1cr)</button>
            </div>
            <pre className="mt-2 min-h-0 flex-1 overflow-auto rounded-lg bg-[#FFFDF6] p-2 font-mono text-[10px] text-[#57534E]">{gitOut || 'git status / diff / commit — Orca-তেই রিভিউ করুন'}</pre>
          </div>
        )}
          ...
          <div className="h-[calc(100%-29px)] overflow-y-auto p-2">
            <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-4">
              {mcpTools.map(t => (
                ...
                }} className="rounded-xl border border-[#D8CDB4] p-2 text-left hover:border-[#0E7C3A]">
        ...
        {tab === 'fleet' && (
          <div className="h-[calc(100%-29px)] overflow-y-auto p-2">
            <p className="mb-1 text-[10px] font-bold text-[#78716C]">BRING YOUR OWN AGENT — {clients.length} clients supported</p>
            <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">
              {clients.map(c => (
                <div key={c.id} className={`rounded-xl border p-2 ${c.id === 'hostamar' || c.id === 'kilocode' ? 'border-[#0E7C3A]' : 'border-[#D8CDB4]'}`}>
                  <p className="text-[10px] font-bold text-[#1C1917]">{c.name} {c.id === 'hostamar' && <span className="text-[8px]" style={{ color: GREEN }}>BUILT-IN</span>}</p>
                  <p className="text-[9px] text-[#57534E]">{c.vendor} — {c.note}</p>
                </div>
              ))}
            </div>
            {fanResults.length > 0 && (
              <div className="mt-2">
                <p className="text-[10px] font-bold text-[#78716C]">FAN RESULTS ({fanResults.length})</p>
                {fanResults.map((r, i) => (
                  <div key={i} className="rounded-lg border border-[#D8CDB4] p-2 text-[10px] text-[#57534E]">
