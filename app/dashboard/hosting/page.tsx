  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1C1917]">হোস্টিং — আপনার সার্ভারসমূহ</h1>
          <p className="text-sm text-[#57534E]">202 queued → auto-provisioned as podman pod + nginx in &lt;30s. ব্যাকআপ to s3.hostamar.com, আপটাইম Kuma, custom domain via Cloudflare Tunnel.</p>
        </div>
      ...
      {loading ? <div className="rounded-xl border bg-[#FFFDF6] p-8 text-center text-[#57534E]">Loading servers...</div>
      : servers.length === 0 ? (
        <div className="rounded-xl border bg-[#FFFDF6] p-10 text-center">
          <p className="font-semibold text-[#1C1917]">এখনো কোনো সার্ভার নেই</p>
          <p className="text-sm text-[#57534E] mt-1">Create one via POST /api/hosting/servers with credits (599 Taka Starter). Queue → pod created.</p>
        ...
        <div className="grid gap-4">
          {servers.map(s=>(
            <div key={s.id} className="rounded-xl border bg-[#FFFDF6] p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-[#1C1917]">{s.name}</h3>
                    ...
                    <span className="text-xs text-[#57534E]">{s.plan || 'custom'} • {s.cpu}vCPU {s.ram}GB {s.storage}GB</span>
                  </div>
                  <p className="text-sm text-[#475569] mt-1">ডোমেইন: <span className="font-medium text-[#1C1917]">{s.domain}</span> • Pod: <code className="bg-[#F1F5F9] px-1 rounded">{s.podName}</code> • পোর্ট: <code className="bg-[#F1F5F9] px-1 rounded">{s.port || '—'}</code> • IP: {s.ip || '—'}</p>
                  <p className="text-xs text-[#57534E] mt-1">আপটাইম: {s.uptime} • ব্যাকআপ: {s.backupAt ? new Date(s.backupAt).toLocaleString() : 'pending — s3.hostamar.com/backups/'+s.id.slice(0,8)+'.tar.gz'} • ID: {s.id.slice(0,8)}</p>
              ...
              {expanded===s.id && (
                <pre className="mt-3 max-h-64 overflow-auto rounded-lg bg-[#FBF4E4] text-[#292524] p-3 text-xs whitespace-pre-wrap">{s.logs || 'No logs yet — podman logs web-'+s.id.slice(0,8)+' | tail -50'}</pre>
              )}
