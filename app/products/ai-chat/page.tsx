...
export default function AIChatProductPage() {
  const p = getProduct('ai-chat')
  if (!p) return null
  return (
    <div className="min-h-screen bg-[#FFFFFF] text-zinc-900 antialiased">
      <div className="max-w-6xl mx-auto px-4 py-10">
        <Link href="/products" className="bangla text-sm text-zinc-500 hover:text-[#2563EB]">&larr; প্রোডাক্ট</Link>
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <Link href={p.ctaHref} className="bangla inline-block px-5 py-2.5 bg-[#2563EB] text-white rounded-full font-semibold hover:bg-[#1D4ED8] transition">{p.ctaLabel}</Link>
          </div>
