...
export default function CurriculumPageClient() {
  ...
  return (
    ...
    >
      <h1>{t('ossu.curriculum')}</h1>
      <p style={{ color: '#666', marginBottom: '2rem' }}>{t('ossu.curriculumDesc')}</p>

      {phases.map((phase) => (
        <div key={phase.id} style={{ marginBottom: '3rem' }}>
          <h2
            style={{ borderBottom: '2px solid #3b82f6', paddingBottom: '0.5rem' }}
          >
            {phase.title}
          </h2>
      ...
      >
        <h2>{t('ossu.finalProject')}</h2>
        <p>{t('ossu.finalProjectDesc')}</p>
        <Link
          href="/ossu/projects"
          style={{
