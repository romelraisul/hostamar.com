...
export default function AdminNodesPage(){
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="overflow-x-auto rounded-2xl bg-[#1C1917] border border-[#0E7C3A]/20">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-zinc-900">
            {nodes.map((n:any)=>(
              <tr key={n.id} className="hover:bg-[#23201D]/40">
                ...
                <td className="px-4 py-3 text-right"><button className="px-3 py-1.5 rounded-lg bg-[#23201D] border border-zinc-800 text-xs text-zinc-300">Restart Tunnel</button> <button className="px-3 py-1.5 rounded-lg bg-[#23201D] border border-zinc-800 text-xs text-zinc-300">View Logs</button></td>
              </tr>
            ))}
          </tbody>
        </table>
