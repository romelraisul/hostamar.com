...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF6] text-[#1C1917]">
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-[#FFFDF6] border-b border-[#0E7C3A]/30 px-4 py-3 flex items-center justify-between">
        ...
        <span className="font-bold text-lg tracking-widest text-[#1C1917]">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-[280px] bg-[#FFFDF6] border-r border-[#0E7C3A]/30 flex flex-col transition-transform duration-200 ${
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="px-6 py-6 border-b border-[#0E7C3A]/20">
            <Link href="/admin?tab=overview" className="flex items-center gap-3">
              ...
              <div>
                <div className="font-black tracking-widest text-[#1C1917] text-[15px] leading-none">HOSTAMAR</div>
                <div className="text-[10px] tracking-[0.2em] text-[#10B981] font-semibold -mt-0.5">ADMIN CONSOLE</div>
              </div>
            </Link>
            <div className="mt-3 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#10B981] animate-pulse" />
              <span className="text-[10px] tracking-widest text-[#57534E]">SYSTEMS ONLINE</span>
            </div>
          </div>
          ...
          <nav className="flex-1 px-3 py-4 space-y-1.5 overflow-y-auto">
            <div className="text-[10px] tracking-[0.18em] text-[#57534E] px-3 mb-2 font-semibold">NAVIGATION</div>
            {navItems.map((item) => {
            ...
            <div className="pt-4 mt-4 border-t border-[#0E7C3A]/10">
              <div className="text-[10px] tracking-[0.18em] text-[#57534E] px-3 mb-2 font-semibold">SYSTEM</div>
              <div className="px-3 py-2 rounded-xl bg-[#0E7C3A]/10 border border-[#0E7C3A]/20">
                <div className="text-xs text-[#57534E]">6 Products · BDIX</div>
                <div className="text-[11px] text-[#57534E]">99.97% uptime · 18-22ms</div>
              </div>
            </div>
          </nav>
            ...
            {userLoading ? (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="h-3 bg-[#FFFDF6] rounded w-3/4 animate-pulse" />
                  <div className="h-2 bg-[#FFFDF6] rounded w-1/2 animate-pulse mt-2" />
                </div>
              </div>
            ) : (
              <>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#57534E] truncate">{user?.email || 'admin@hostamar.com'}</p>
                    <p className="text-[10px] text-[#57534E] truncate mt-0.5 font-mono">ID: {(user?.id || '—').slice(0, 12)}</p>
                  </div>
                </div>
                <button
                  onClick={handleLogout}
                  className="flex items-center gap-2 text-[#78716C] hover:text-white w-full px-3 py-2 rounded-xl hover:bg-[#0E7C3A]/10 border border-transparent hover:border-[#0E7C3A]/20 transition-colors"
                >
        ...
        <div className="pt-[56px] lg:pt-0">
          ...
          <div className={`sticky top-0 lg:top-0 z-30 flex items-center justify-between px-4 py-2 border-b backdrop-blur ${live === 'offline' ? 'bg-red-950/80 border-red-500/40' : 'bg-black/80 border-[#0E7C3A]/20'}`}>
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${live === 'online' ? 'bg-[#10B981] animate-pulse' : live === 'offline' ? 'bg-red-500' : 'bg-[#F6EBD2] animate-pulse'}`} />
              <span className={`text-[11px] font-bold tracking-widest ${live === 'online' ? 'text-[#10B981]' : live === 'offline' ? 'text-red-400' : 'text-[#57534E]'}`}>
                {live === 'online' ? 'LIVE' : live === 'offline' ? 'OFFLINE' : 'CHECKING'}
              </span>
              <span className="text-[10px] text-[#57534E] font-mono hidden sm:inline">hostamar.com · auto-refresh on</span>
            </div>
            <span className="text-[10px] text-[#57534E] font-mono">{clock ? `updated ${clock}` : '…'}</span>
          </div>
          <div className="min-h-screen bg-[#FFFDF6]">{children}</div>
        </div>
      </div>
    </div>
  )
