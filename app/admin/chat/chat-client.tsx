  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-zinc-950 text-zinc-100">
      ...
      <div className="hidden lg:flex w-72 border-r border-zinc-800 flex-col p-3 gap-2 overflow-y-auto">
        ...
        <button onClick={toggleAutonomous} className={`mt-2 text-xs px-2 py-1.5 rounded border font-semibold ${autonomous?'bg-emerald-600 text-white border-emerald-500':'bg-[#23201D] text-zinc-400 border-zinc-800'}`}>
        ...
        <div className="flex gap-1 mt-2">
          {(['all','founder','support'] as const).map(f=>(
            <button key={f} onClick={()=>setFilter(f)} className={`text-[10px] px-2 py-1 rounded border ${filter===f?'bg-zinc-800 text-white':'bg-[#23201D] text-zinc-500'}`}>{f==='support'?'Customer':f==='founder'?'Founder':'All'}</button>
          ))}
        </div>
        <div className="relative mt-1">
          <Search size={12} className="absolute left-2 top-2 text-zinc-600"/>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search history..." className="w-full pl-7 pr-2 py-1.5 text-xs bg-[#23201D] border border-zinc-800 rounded"/>
        </div>
        <div className="mt-2 text-xs font-semibold text-zinc-300">Slash Commands</div>
        {SLASH.map(c=>(
          <button key={c} onClick={()=>setInput(c+' ')} className="text-left text-xs px-2 py-1.5 rounded bg-[#23201D] hover:bg-zinc-800 border border-zinc-800">{c}</button>
        ))}
        ...
        <div className="space-y-1.5 max-h-[40vh] overflow-y-auto pr-1">
          {filtered.slice(0,30).map((h:any)=>(
            <button key={h.id} onClick={()=>setMessages(m=>[...m,{role:h.role,content:h.content.slice(0,2000)}])} className="text-left w-full p-2 rounded bg-[#23201D] border border-zinc-800 hover:bg-zinc-800">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {messages.map((m,i)=>(
            <div key={i} className={`flex gap-2 ${m.role==='user'?'justify-end':'justify-start'}`}>
              <div className={`max-w-[85%] rounded-xl px-3 py-2 text-sm whitespace-pre-wrap break-words ${m.role==='user'?'bg-blue-600 text-white':'bg-[#23201D] border border-zinc-800'}`}>
        ...
        <div className="border-t border-zinc-800 p-3 flex gap-2">
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey} placeholder="Type /check, /audit, /build bKash, or ask anything…" className="flex-1 bg-[#23201D] border border-zinc-800 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-600" />
...
function StatusCard({icon,title,value,sub}:{icon:any,title:string,value:string,sub:string}){
  return <div className="rounded-lg border border-zinc-800 bg-[#23201D] p-2.5">
