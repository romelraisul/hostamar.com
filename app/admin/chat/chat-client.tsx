...
export default function ChatOsClient({ user }: { user: any }) {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-[#FBF4E4] text-[#1C1917]">
      {/* Left: history */}
      <div className="hidden lg:flex w-72 border-r border-[#D8CDB4] flex-col p-3 gap-2 overflow-y-auto">
        ...
        <div className="flex gap-1 mt-2">
          {(['all','founder','support'] as const).map(f=>(
            <button key={f} onClick={()=>setFilter(f)} className={`text-[10px] px-2 py-1 rounded border ${filter===f?'bg-[#FFFDF6] text-[#1C1917]':'bg-[#FDF8EC] text-[#57534E]'}`}>{f==='support'?'Customer':f==='founder'?'Founder':'All'}</button>
          ))}
        </div>
        <div className="relative mt-1">
          <Search size={12} className="absolute left-2 top-2 text-[#57534E]"/>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search history..." className="w-full pl-7 pr-2 py-1.5 text-xs bg-[#FDF8EC] border border-[#D8CDB4] rounded"/>
        </div>
        <div className="mt-2 text-xs font-semibold text-[#57534E]">Slash Commands</div>
        {SLASH.map(c=>(
          <button key={c} onClick={()=>setInput(c+' ')} className="text-left text-xs px-2 py-1.5 rounded bg-[#FDF8EC] hover:bg-[#FFFDF6] border border-[#D8CDB4]">{c}</button>
        ))}
        <div className="mt-3 text-xs font-semibold text-[#57534E]">History ({filtered.length})</div>
        <div className="space-y-1.5 max-h-[40vh] overflow-y-auto pr-1">
          {filtered.slice(0,30).map((h:any)=>(
            <button key={h.id} onClick={()=>setMessages(m=>[...m,{role:h.role,content:h.content.slice(0,2000)}])} className="text-left w-full p-2 rounded bg-[#FDF8EC] border border-[#D8CDB4] hover:bg-[#FFFDF6]">
              <div className="flex gap-1 text-[10px] text-[#57534E]"><span className={h.customerId==='support-widget'?'text-blue-400':'text-emerald-400'}>{h.customerId==='support-widget'?'💬':'👑'} {h.customerId.slice(0,12)}</span><span>{new Date(h.createdAt).toLocaleDateString()}</span></div>
              <div className="text-xs text-[#57534E] line-clamp-2">{h.content.slice(0,120)}</div>
            </button>
          ))}
          {filtered.length===0 && <div className="text-[11px] text-[#57534E]">No history — try /check</div>}
        </div>
        <button onClick={()=>{setMessages([{role:'assistant',content:'Cleared.'}]); localStorage.removeItem('hostamar_chat_history')}} className="mt-2 text-xs text-[#57534E] hover:text-[#57534E]">Clear local</button>
        <button onClick={fetchHistory} className="text-xs text-[#57534E] hover:text-[#57534E]">↻ Refresh DB history</button>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {messages.map((m,i)=>(
            <div key={i} className={`flex gap-2 ${m.role==='user'?'justify-end':'justify-start'}`}>
              <div className={`max-w-[85%] rounded-xl px-3 py-2 text-sm whitespace-pre-wrap break-words ${m.role==='user'?'bg-blue-600 text-white':'bg-[#FDF8EC] border border-[#D8CDB4]'}`}>
                <div className="flex items-center gap-1.5 text-[10px] opacity-60 mb-1">{m.role==='user'?<User size={11}/>:<Bot size={11}/>}{m.role}</div>
                {m.content}
              </div>
            </div>
          ))}
          {loading && <div className="text-xs text-[#57534E] animate-pulse">… thinking — running CHECK + best free model</div>}
          <div ref={bottomRef} />
        </div>
        <div className="border-t border-[#D8CDB4] p-3 flex gap-2">
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey} placeholder="Type /check, /audit, /build bKash, or ask anything…" className="flex-1 bg-[#FDF8EC] border border-[#D8CDB4] rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-600" />
        ...
        <div className="px-3 py-1.5 text-[10px] text-[#57534E] border-t border-[#D8CDB4] flex gap-3">
      ...
      {/* Right: live status */}
      <div className="hidden xl:flex w-80 border-l border-[#D8CDB4] flex-col overflow-y-auto p-3 gap-3 bg-[#FBF4E4]">
        <div className="text-xs font-bold text-[#78716C] uppercase">Live Status</div>
        ...
        <div className="text-[11px] text-[#57534E] border-t border-[#D8CDB4] pt-2">
...
function StatusCard({icon,title,value,sub}:{icon:any,title:string,value:string,sub:string}){
  return <div className="rounded-lg border border-[#D8CDB4] bg-[#FDF8EC] p-2.5">
    <div className="flex items-center gap-1.5 text-[11px] font-semibold text-[#57534E]">{icon}{title}</div>
    <div className="text-xs text-[#1C1917] mt-1 break-words">{value}</div>
    <div className="text-[10px] text-[#57534E] truncate">{sub}</div>
  </div>
}
