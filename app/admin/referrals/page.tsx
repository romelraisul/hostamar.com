...
export default function AdminReferralsPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      ...
      <p className="bangla text-sm text-[#57534E]">প্রতি সফল রেফারেলে রেফারার পায় ৫০০ ক্রেডিট + ৬০ টাকা — পেন্ডিং থেকে পেইড করুন।</p>
      ...
      <div className="flex gap-2 mt-4">
        {[
          ...
          <button key={k} onClick={()=>setFilter(k)} className={`bangla px-4 py-2 rounded-full text-sm border ${filter===k?'bg-[#0E7C3A] text-white border-[#0E7C3A]':'bg-[#FFFDF6] border-[#E6DCC4]'}`}>{l}</button>
        ))}
        <button onClick={load} className="bangla ml-auto px-4 py-2 rounded-full bg-[#FFFDF6] border border-[#E6DCC4] text-sm">রিফ্রেশ</button>
      </div>

      {msg && <div className="mt-3 p-3 rounded-xl bg-amber-50 border border-amber-200 text-sm">{msg}</div>}

      <div className="mt-4 rounded-2xl border border-[#E6DCC4] bg-[#FFFDF6] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} className="p-8 text-center text-[#57534E] bangla">লোড হচ্ছে...</td></tr>
              ) : rows.length===0 ? (
                <tr><td colSpan={7} className="p-8 text-center text-[#57534E] bangla">কোনো রেফারেল নেই</td></tr>
              ) : rows.map(r=> (
                <tr key={r.id} className="border-t border-[#E6DCC4]">
                  <td className="px-3 py-2"><div className="font-medium">{r.referrerName}</div><div className="text-xs text-[#57534E]">{r.referrerEmail}</div></td>
                  <td className="px-3 py-2"><div className="font-medium">{r.referredName}</div><div className="text-xs text-[#57534E]">{r.referredEmail}</div></td>
                  ...
                  <td className="px-3 py-2 text-xs text-[#57534E]">{new Date(r.createdAt).toLocaleDateString('bn-BD')}</td>
                  <td className="px-3 py-2">
                    {['pending','PENDING'].includes(r.status) ? (
                      <button disabled={approving===r.id} onClick={()=>approve(r.id)} className="bangla px-3 py-1.5 rounded-full bg-[#0E7C3A] text-white text-xs font-semibold disabled:bg-[#F0E7CF]">
                        {approving===r.id ? '...' : 'পেআউট অনুমোদন'}
                      </button>
                    ) : <span className="bangla text-xs text-[#78716C]">সম্পন্ন</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bangla mt-4 p-4 rounded-xl bg-[#0E7C3A]/10 border border-[#0E7C3A]/20 text-sm text-[#44403C]">
