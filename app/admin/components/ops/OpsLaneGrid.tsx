...
function verdictStyle(v?: string | null) {
  if (!v) return 'bg-[#FFFDF6] text-[#78716C] border-[#D8CDB4]'
...
export default function OpsLaneGrid({
  ...
  if (!lanes.length) {
    return (
      <div className="rounded-2xl bg-[#FFFDF6] border border-[#0E7C3A]/20 p-10 text-center">
        {err ? <div className="text-sm text-red-300">{err}</div>
          : loading ? <div className="text-sm text-[#57534E]">Loading lanes…</div>
          : <div className="text-sm text-[#57534E]">No fleet lanes registered yet — they appear once employees post their first run.</div>}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="text-xs tracking-[0.2em] text-[#57534E]">LANES · {lanes.length}</div>
        <div className="text-[10px] text-[#57534E]">{lanes.filter((l) => !l.paused).length} active · {lanes.filter((l) => l.paused).length} paused</div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        {lanes.map((l) => (
          <div key={l.employee} className={`rounded-xl bg-[#FFFDF6] border p-3 transition ${l.paused ? 'border-amber-500/30' : 'border-[#D8CDB4] hover:border-[#10B981]/30'}`}>
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${dotStyle(l)}`} />
                <span className="text-sm font-bold text-[#1C1917] truncate">{l.employee}</span>
              </div>
            ...
            <div className="text-[11px] text-[#57534E] mt-1 truncate">{l.role || '—'}</div>
            <div className="text-[10px] text-[#57534E] truncate">{l.schedule || 'no schedule'}</div>
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-[10px] text-[#57534E] font-mono ml-auto">{ageLabel(l.ageMinutes)}</span>
            </div>
            {l.lastSnippet && <div className="text-[11px] text-[#78716C] mt-2 line-clamp-2 break-words">{l.lastSnippet}</div>}
            {l.paused && <div className="text-[10px] text-amber-300 mt-2">⏸ paused — runs disabled by control sync</div>}
            <div className="flex gap-2 mt-3">
              <button onClick={() => onSelect(l.employee)} className="flex-1 py-1.5 rounded-lg border border-[#D8CDB4] text-[11px] text-[#57534E] hover:border-[#10B981] hover:text-[#1C1917] transition flex items-center justify-center gap-1">
