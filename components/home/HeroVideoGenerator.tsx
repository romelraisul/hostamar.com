...
export default function HeroVideoGenerator(){
  ...
  const src = `/api/showcase/${showcaseId}/video`
  return (
      ...
      <div className="max-w-[1120px] mx-auto px-4 sm:px-5">
        ...
        <div className="mt-4 grid md:grid-cols-[30%_70%] gap-4">
          <div className="space-y-3">
            ...
            <video src="/showcase/eid.mp4" poster="" className="w-full h-24 object-cover rounded-xl border border-[#0E7C3A]/20 hidden md:block" muted loop playsInline autoPlay controls={false} />
          </div>
          <div className="relative aspect-video rounded-2xl overflow-hidden border-2 border-[#0E7C3A] bg-black">
            <video src={hero==='latest' ? src : hero==='eid' ? '/showcase/eid.mp4' : hero==='boishakh' ? '/showcase/boishakh.mp4' : '/showcase/11-11.mp4'} poster="" className="w-full h-full object-cover" muted loop playsInline autoPlay controls />
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          <button onClick={()=>setHero('eid')} className={`border rounded-xl p-3 bg-white hover:bg-[#F0FDF4] text-left ${hero==='eid'?'border-[#0E7C3A] ring-1 ring-[#0E7C3A]': 'border-[#0E7C3A]/20'}`}>
            ...
            <video src="/showcase/eid.mp4" className="mt-2 w-full h-20 object-cover rounded-lg border" muted loop playsInline />
          </button>
          <button onClick={()=>setHero('boishakh')} className={`border rounded-xl p-3 bg-white hover:bg-[#F0FDF4] text-left ${hero==='boishakh'?'border-[#0E7C3A] ring-1 ring-[#0E7C3A]': 'border-[#0E7C3A]/20'}`}>
            ...
            <video src="/showcase/boishakh.mp4" className="mt-2 w-full h-20 object-cover rounded-lg border" muted loop playsInline />
          </button>
          <button onClick={()=>setHero('sale')} className={`border rounded-xl p-3 bg-white hover:bg-[#F0FDF4] text-left ${hero==='sale'?'border-[#0E7C3A] ring-1 ring-[#0E7C3A]': 'border-[#0E7C3A]/20'}`}>
            ...
            <video src="/showcase/11-11.mp4" className="mt-2 w-full h-20 object-cover rounded-lg border" muted loop playsInline />
          </button>
        </div>
      </div>
    </section>
