...
# Load .env.marketing if present
try:
    from dotenv import load_dotenv
    env_path = os.path.join(os.path.dirname(__file__), ".env.marketing")
    if os.path.exists(env_path):
        load_dotenv(env_path)
        print(f"   📄 Loaded env from: {env_path}")
    else:
        # Also check current working directory
        cwd_env = os.path.join(os.getcwd(), ".env.marketing")
...
CONFIG = {
    ...
    "facebook_groups_csv": os.environ.get("FACEBOOK_GROUPS", ""),
}

# Output directory – defaults to ./marketing-output/ relative to this script
OUTPUT_BASE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "marketing-output")
OUTPUT_DIR = os.environ.get("MARKETING_OUTPUT_DIR", OUTPUT_BASE)
...
FACEBOOK_GROUPS = [
    {
        ...
        "id": os.environ.get("FACEBOOK_GROUP_1", "FILL_IN_GROUP_ID_1"),
        "members": "100K+",
        "category": "youtube_creators"
    },
    {
        "name": "Facebook Video Editors BD",
        "id": os.environ.get("FACEBOOK_GROUP_2", "FILL_IN_GROUP_ID_2"),
        "members": "50K+",
        "category": "video_editors"
    },
    {
        "name": "Content Creators Bangladesh",
        "id": os.environ.get("FACEBOOK_GROUP_3", "FILL_IN_GROUP_ID_3"),
        "members": "75K+",
        "category": "content_creators"
    },
    {
        "name": "Digital Marketing Bangladesh",
        "id": os.environ.get("FACEBOOK_GROUP_4", "FILL_IN_GROUP_ID_4"),
        "members": "60K+",
        "category": "digital_marketing"
    },
    {
        "name": "Startup Bangladesh",
        "id": os.environ.get("FACEBOOK_GROUP_5", "FILL_IN_GROUP_ID_5"),
        "members": "80K+",
        "category": "startups"
    }
]
...
def generate_all_content():
    ...
    for post in MARKETING_POSTS:
        filepath = f"{OUTPUT_DIR}/fb_{post['id']}_{post['type']}.txt"
    ...
    for i, (group, msg) in enumerate(zip(WHATSAPI_GROUPS, WHATSAPP_MESSAGES)):
        for msg_idx, message in enumerate(WHATSAPP_MESSAGES):
            filepath = f"{OUTPUT_DIR}/wa_{group.replace(' ', '_')[:30]}_{msg_idx}.txt"
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(f"GROUP: {group}\n\n{message}")
    
    # 3. Email templates
    for template_name, template in EMAIL_TEMPLATES.items():
        filepath = f"{OUTPUT_DIR}/email_{template_name}.txt"
        with open(filepath, 'w', encoding='utf-8') as f:
    ...
    for i, video in enumerate(YOUTUBE_VIDEOS):
        filepath = f"{OUTPUT_DIR}/yt_{i+1}_{video['title'][:40].replace(' ', '_')}.txt"
        with open(filepath, 'w', encoding='utf-8') as f:
...
def whatsapp_broadcast():
    ...
    for i, (group, msg) in enumerate(zip(WHATSAPI_GROUPS, WHATSAPP_MESSAGES)):
        ...
        filepath = f"{OUTPUT_DIR}/whatsapp_ready_{i+1}.txt"
...
def email_campaign():
    ...
    for name, template in EMAIL_TEMPLATES.items():
        filepath = f"{OUTPUT_DIR}/email_{name}_ready.txt"
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(f"SUBJECT: {template['subject']}\n\n{template['body']}")
...
def youtube_upload():
    ...
    print("\n🎬 YouTube Content...")
    for i, video in enumerate(YOUTUBE_VIDEOS):
        filepath = f"{OUTPUT_DIR}/yt_upload_{i+1}_ready.txt"
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(f"TITLE: {video['title']}\n")
            f.write(f"TAGS: {', '.join(video['tags'])}\n")
            f.write(f"DESCRIPTION:\n{video['description']}")
